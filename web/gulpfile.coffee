...
gulp.task 'webpack-lib', ['assemble', 'webpack-config'], ->
...
gulp.task 'watch', ['default'], ->
  ...
  gulp.watch(watched_paths, ['dist'])

# blow away all the built stuff
gulp.task 'clean', ->
  del.sync [DIST_DIR, BUILD_TMP], # synchronous delete
    force: true

# builds all code into a testable form into build_tmp
gulp.task 'compile', ['compile-coffee', 'compile-ts']

gulp.task 'assemble', ['assets', 'blueprint-resources', 'compile-sass']

# constructs the full web tree
gulp.task 'dist', ['compile', 'assemble', 'webpack-lib', 'webpack-app']
